...
#define         UART0_INT_LEVEL 1       /* UART0割り込みレベル 1～7     */
...
int write( int c )
{
    ...
    if( (s0tic & 0x07) == 0 ) {
        ...
        s0tic = UART0_INT_LEVEL;        /* 送信割り込み優先レベル設定   */
        put_uart0( put_data );
    }
...
2011.04.01 Ver.1.21 write関数内で、送信割り込み優先レベルの設定の位置変更
                    割り込み送信レベル7→1に変更
2012.03.12 Ver.1.22 コンパイラVer.6.00.00対応
*/
